...
func (v *Volume) SetXAttr(path string, key string, data []byte, autoCreate bool) error {
	...
	if err == syscall.ENOENT {
		...
		if inodeInfo, err = v.mw.Create_ll(parentID, filename, DefaultFileMode, 0, 0, nil, path, meta.CreateOptions{}); err != nil {
			return err
		}
...
func (v *Volume) recursiveMakeDirectory(path string) (partentIno uint64, err error) {
	...
	for pathIterator.HasNext() {
		...
		if err == syscall.ENOENT {
			var info *proto.InodeInfo
			info, err = v.mw.Create_ll(partentIno, pathItem.Name, uint32(DefaultDirMode), 0, 0, nil, path[:pathIterator.cursor], meta.CreateOptions{})
...
func (v *Volume) lookupDirectories(dirs []string, autoCreate bool) (inode uint64, err error) {
	...
	for _, dir := range dirs {
		...
		if lookupErr == syscall.ENOENT {
			var inodeInfo *proto.InodeInfo
			var createErr error
			inodeInfo, createErr = v.mw.Create_ll(parentId, dir, uint32(DefaultDirMode), 0, 0, nil, "/"+dir, meta.CreateOptions{})
