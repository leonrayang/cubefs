...
type  repairWorker struct{
	...
	shallDegrade bool
	AfterPre     bool
	packChannel  chan repl.PacketEx
	dp           *DataPartition
...
func reciverMakeTinyPacket(partitionID uint64, extentID uint64, offset, size int) (p repl.PacketEx) {
	return &repairWorker{}
}

func reciverMakeNormalPacket(partitionID uint64, extentID uint64, offset, size int) (p repl.PacketEx) {
...
func reciverMakeExtentWithHoleRepairReadPacket(partitionID uint64, extentID uint64, offset, size int) (p repl.PacketEx) {
	recvWorker.ExtentOffset = int64(offset)
	recvWorker.Size = uint32(size)
	recvWorker.PartitionID = partitionID
	recvWorker.ExtentID = extentID
	recvWorker.Opcode = proto.OpNormalWithHoleExtentRepairRead

	return recvWorker
}

func reciverNewPacket() (p repl.PacketEx) {
	pr := recvWorker
...
func sendNewHoleReadResponsePacket(requestID int64, partitionID uint64, extentID uint64) (p repl.PacketEx) {
	pr := sendWorker
...
func sendNewNormalReadResponsePacket(requestID int64, partitionID uint64, extentID uint64) (p repl.PacketEx) {
	pr := sendWorker
	pr.ExtentID = extentID
	pr.PartitionID = partitionID
	pr.Magic = proto.ProtoMagic
...
func mockInitWorker(t *testing.T, role string) *repairWorker{
	...
	worker.packChannel = make(chan repl.PacketEx, 100)
	...
	worker.dp.disk, err = NewDisk("/tmp", 200, 2000, 10, spaceManager)
	require.NoError(t, err)
...
func senderRepairWorker(t *testing.T, exitCh chan struct{}) {
	...
	for {
		...
		case pr := <-sendWorker.packChannel:
			if pr.GetOpcode() == proto.OpNormalWithHoleExtentRepairRead {
				t.Logf("TestExtentRepair role %v recive packet (%v)", "sender", pr)
				getReplyPacket := func() repl.PacketEx {
...
func TestExtentRepair(t *testing.T) {
	...
	defer func() {
		os.RemoveAll(sendWorker.dp.path)
		os.RemoveAll(recvWorker.dp.path)
	}()
