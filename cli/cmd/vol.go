...
const (
	...
	cmdVolDefaultRemoteCacheMaxFileSizeMB = proto.DefaultRemoteCacheMaxFileSizeMB
...
func newVolCreateCmd(client *master.MasterClient) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if optRemoteCacheMaxFileSizeMB <= 0 {
...
func newVolUpdateCmd(client *master.MasterClient) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if cmd.Flags().Changed(CliFlagRemoteCacheMaxFileSizeMB) && optRemoteCacheMaxFileSizeMB <= 0 {
				err = fmt.Errorf("param remoteCacheMaxFileSizeMB(%v) must greater than 0", optRemoteCacheMaxFileSizeMB)
				return
			}
	...
	cmd.Flags().Int64Var(&optRemoteCacheSameRegionTimeout, CliFlagRemoteCacheSameRegionTimeout, proto.DefaultRemoteCacheSameRegionTimeout, "Remote cache same region timeout millisecond(must > 0),default 100")
